...
import { Search, Loader2, Wallet, Coins, ArrowRight, Activity, CheckCircle2 } from 'lucide-react';
...
// Memoized Wallet Player Row Component
const WalletPlayerRow = memo(({ player, isExpanded, isLast, lastElementRef, onWalletInfoClick, walletData, walletLoading }) => {
  ...
  return (
    <div ref={isLast ? lastElementRef : null} className={`flex flex-col transition-all duration-300 ${isExpanded ? 'bg-white/[0.03]' : ''}`}>
      {/* Player Row - Mobile Stacked / Desktop Grid */}
      <div className="flex flex-col md:grid md:grid-cols-12 md:items-center gap-4 px-4 py-5 md:px-6 hover:bg-white/[0.04] transition-all group cursor-pointer" onClick={() => onWalletInfoClick(player)}>
        ...
        <div className="col-span-3 flex justify-end items-center mt-2 md:mt-0">
          <button
            onClick={(e) => { e.stopPropagation(); onWalletInfoClick(player); }}
          ...
          >
            <Wallet size={16} />
            {isExpanded ? 'Hide Wallet Info' : 'Wallet Info'}
          </button>
        </div>
      </div>

      {/* Expandable Wallet Data Mobile-first Cards */}
      <AnimatePresence>
        {isExpanded && (
          <M.div
            ...
            <div className="px-4 py-6 sm:px-6 md:px-10">
              <div className="flex items-center gap-3 mb-6">
                <div className="p-2 bg-prime/20 rounded-lg">
                  <Wallet size={20} className="text-prime" />
                </div>
                <h3 className="text-lg font-bold">
                  Wallet Details for <span className="text-prime truncate max-w-[150px] sm:max-w-xs md:max-w-md inline-block align-bottom">{name}</span>
                </h3>
              </div>
              ...
              {walletLoading ? (
                ...
                <div className="bg-white/[0.02] border border-white/5 rounded-2xl p-10 text-center">
                  ...
                  <p className="text-white/40 font-medium">No wallet data found for this player.</p>
                </div>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                  {walletData.map((item, idx) => (
                    <div key={item.id || item._id || item.currency || `walletData-${idx}`} className="bg-white/[0.02] border border-white/5 rounded-xl p-5 space-y-3 hover:bg-white/[0.04] transition-colors group">
                      {Array.from(new Set(walletData.flatMap(Object.keys))).map(key => {
...
const WalletPage = () => {
  ...
  const handleWalletInfoClick = useCallback(async (player) => {
    ...
    setExpandedRow(prev => {
      ...
      const fetchWallet = async () => {
        ...
        try {
          const data = await authService.getUserWallets(id);
          const wallets = data.wallets || (Array.isArray(data) ? data : [data]);
          setWalletData(wallets.length ? wallets : []);
        } catch (err) {
          console.error(err);
          setWalletData([]);
        } finally {
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-1000">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl sm:text-3xl font-bold tracking-tight">Player Wallets</h2>
          <p className="text-white/50 mt-1">View wallet balances and currencies for registered players.</p>
        </div>
        ...
        <div className="divide-y divide-white/5">
          {loading ? (
            ...
            players.map((player, index) => (
              <WalletPlayerRow
                key={safeGet(player, 'id', ['_id', 'userId'], `wallet-${index}`)}
                player={player}
